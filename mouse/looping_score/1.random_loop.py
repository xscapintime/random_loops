...
for f in file:
    ...
    if  re.match('.*_r1.*', f) != None:
        fn = os.path.split(f)[1].split('.esc')[0].replace("_shuff", "")
        filename_r1.append(fn + '_r1')
    else:
        fn = os.path.split(f)[1].split('.esc')[0].replace("_shuff", "")
        filename_r2.append(fn + '_r2')

print(len(filename_r1))
print(len(filename_r2))
...
for i in np.linspace(0, back_r1.shape[1]-5, int(len(file)/10), dtype=int):
    #print(i)
    #print(i, i+5)
