...
dat <- read.csv("../looping_score_v2/zmat_thresholds.txt", sep = "\t", header = T)

fin_df <- read.csv("../looping_score_v2/contact_zscore.fin.txt", sep = "\t", header = T, row.names = 1)
...
## split into 3 groups
mk1 <- fin_df["BCL11A_46178", "zscore"]
mk2 <- fin_df["CREB1_46182", "zscore"]
...
ggsave(filename = "hisogram_all_zscore_v3.png", p_fixed, path = "../figs/", width = 7, height = 7)
ggsave(filename = "hisogram_all_zscore_v3.pdf", p_fixed, path = "../figs/", width = 7, height = 7)